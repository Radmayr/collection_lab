...
## Графики в ClearML не сохраняются

`Logger.report_plotly` не выбрасывает исключений, даже если сервер отверг данные: слишком большой
запрос (лимит сервера или прокси) молча отбрасывается, а в лог попадает одна строка
`ERROR ... request exceeds limit`. Что делает библиотека и что делать вам:

1. Графики библиотеки компактные (гистограммы — предподсчитанные бины, а не все значения).
2. Фигуры больше лимита (по умолчанию 5 МБ) прореживаются с предупреждением. Лимит:
   переменная окружения `COLLECTION_LAB_CLEARML_MAX_MB`.
3. `Experiment.close()` сверяет с сервером, какие графики дошли, и выводит предупреждение
   с названиями недошедших и текстом серверной ошибки.
4. Чтобы узнать реальный лимит вашего сервера, запустите там же, где не сохраняется:

```python
from collection_lab.tracking import clearml as cml
cml.diagnose()      # отправит тестовые графики 0.05…14 МБ и покажет, какие дошли
```
